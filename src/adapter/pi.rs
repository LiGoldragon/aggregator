...
use crate::{
    ...
    adapter::{
        ...
        TranscriptFileDescriptor, TranscriptFileDiscovery, TranscriptFileSync,
        TranscriptJsonMetadata, TranscriptLineLocator, TranscriptRawReadOutcome,
        TranscriptReadOutcome, TranscriptReadRequest, TranscriptRecord, TranscriptRecordLifecycle,
        TranscriptRecordSink, TranscriptResumableScanOutcome, TranscriptScanCursor,
        TranscriptScanLimits, TranscriptScanRequest,
    },
...
impl PiRunHistoryRootReader {
    ...
    /// Compatibility-source lifecycle scan. The cursor is issued only after synced file events.
    pub fn scan_records_resumable<S: TranscriptRecordSink>(
        ...
        let source_identifier = self.source_identifier();
        sink.begin_source(SourceKind::Pi, request.configured_occurrence);
        ...
        TranscriptResumableScanOutcome {
            ...
            resumed: request.accepts(SourceKind::Pi, &self.source_identifier()),
            outcome,
        }
    }
    ...
    pub fn read_file_lines<S: TranscriptRecordSink>(
        ...
        let mut receive = |bounded_line| match bounded_line {
            TranscriptBoundedLine::Text { line_number, text } => {
                match PiJsonlRecord::new(&text).into_transcript_record(
                    file.to_path_buf(),
                    line_number,
                    self.source_identifier(),
                ) {
                    PiJsonlRecordResult::Record(record) => sink.observe_record(record),
                    PiJsonlRecordResult::Malformed => failures.push(self.failure(
            ...
            TranscriptBoundedLine::Truncated(truncation) => {
                ...
                truncations.push(truncation.clone().into_truncation(SourceKind::Pi));
